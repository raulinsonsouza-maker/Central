          ...
          <CardHeader className="border-b border-[var(--border)]/60 px-6 pb-5 pt-6 sm:px-8">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <h3 className="text-xl font-black uppercase tracking-tight text-[var(--foreground)] sm:text-2xl">
                  Campanhas com{" "}
                  <span className="bg-[linear-gradient(90deg,var(--accent),var(--primary))] bg-clip-text text-transparent">
                    Vendas
                  </span>
                </h3>
                <p className="mt-1 text-sm text-[var(--muted-foreground)]">
                  Todas as campanhas que geraram vendas no período selecionado.
                </p>
              </div>
              <span className="rounded-full border border-[var(--primary)]/25 bg-[var(--primary)]/12 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.16em] text-[var(--foreground)]">
                {data.campanhas.length} {data.campanhas.length === 1 ? "campanha" : "campanhas"}
              </span>
            </div>
          </CardHeader>
          <CardContent className="px-3 pb-4 pt-4 sm:px-5 sm:pb-5">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[860px] border-separate [border-spacing:0_8px]">
                <thead>
                  <tr>
                    {[
                      { label: "Campanha", align: "left", w: "min-w-[260px]" },
                      { label: "Investido", align: "right", w: "" },
                      { label: "Vendas", align: "right", w: "" },
                      { label: "Custo por Venda", align: "right", w: "" },
                      { label: "Ticket Médio", align: "right", w: "" },
                      { label: "Total Faturado", align: "right", w: "" },
                    ].map((col) => (
                      <th
                        key={col.label}
                        className={`px-4 pb-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--muted-foreground)] text-${col.align} ${col.w}`}
                      >
                        {col.label}
                      </th>
                    ))}
                  ...
                  {data.campanhas.map((c, i) => {
                    ...
                    return (
                      <tr key={c.nome} className="group">
                        <td className={`rounded-l-2xl px-4 py-4 ${isTop ? "bg-[var(--primary)]/8" : "bg-white/[0.03]"}`}>
                        ...
                        <td className={`px-4 py-4 text-right tabular-nums text-[13px] text-[var(--muted-foreground)] ${isTop ? "bg-[var(--primary)]/8" : "bg-white/[0.03]"}`}>
                          {formatCurrency(c.investimento)}
                        </td>
                        <td className={`px-4 py-4 text-right tabular-nums ${isTop ? "bg-[var(--primary)]/8" : "bg-white/[0.03]"}`}>
                          <span className={`text-[15px] font-black ${isTop ? "text-[var(--primary)]" : "text-[var(--foreground)]"}`}>
                            {formatInteger(c.vendas)}
                          </span>
                        </td>
                        <td className={`px-4 py-4 text-right tabular-nums text-[13px] text-[var(--muted-foreground)] ${isTop ? "bg-[var(--primary)]/8" : "bg-white/[0.03]"}`}>
                          {formatCurrency(c.custoporVenda)}
                        </td>
                        <td className={`px-4 py-4 text-right tabular-nums text-[13px] text-[var(--muted-foreground)] ${isTop ? "bg-[var(--primary)]/8" : "bg-white/[0.03]"}`}>
                          {formatCurrency(c.ticketMedio)}
                        </td>
                        <td className={`rounded-r-2xl px-4 py-4 text-right tabular-nums ${isTop ? "bg-[var(--primary)]/8" : "bg-white/[0.03]"}`}>
                ...
                <tfoot>
                  <tr>
                    ...
                    <td className="bg-white/[0.06] px-4 py-3 text-right tabular-nums text-[13px] font-semibold text-[var(--foreground)]">
                      {formatCurrency(data.campanhas.reduce((s, c) => s + c.investimento, 0))}
                    </td>
                    <td className="bg-white/[0.06] px-4 py-3 text-right tabular-nums text-[15px] font-black text-[var(--primary)]">
                      {formatInteger(data.campanhas.reduce((s, c) => s + c.vendas, 0))}
                    </td>
                    <td className="bg-white/[0.06] px-4 py-3 text-right tabular-nums text-[13px] font-semibold text-[var(--foreground)]">
                      {formatCurrency(
                        data.campanhas.reduce((s, c) => s + c.investimento, 0) /
                          Math.max(1, data.campanhas.reduce((s, c) => s + c.vendas, 0))
                      )}
                    </td>
                    <td className="bg-white/[0.06] px-4 py-3 text-right tabular-nums text-[13px] font-semibold text-[var(--foreground)]">
                      {formatCurrency(
                        data.campanhas.reduce((s, c) => s + c.faturamento, 0) /
                          Math.max(1, data.campanhas.reduce((s, c) => s + c.vendas, 0))
                      )}
                    </td>
                    <td className="rounded-r-2xl bg-white/[0.06] px-4 py-3 text-right tabular-nums text-[13px] font-bold text-[var(--primary)]">
                      {formatCurrency(data.campanhas.reduce((s, c) => s + c.faturamento, 0))}
                    </td>
                  </tr>
                </tfoot>
              </table>
